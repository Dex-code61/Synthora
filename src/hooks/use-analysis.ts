...
export function useStartAnalysis() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['analysis-status', data.jobId] })
      queryClient.invalidateQueries({ queryKey: ['analysis-results', data.repositoryId.toString()] })
    },
  })
}
...
export function useHotspots(repositoryId: string, threshold: number = 0.7) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/repositories/${repositoryId}/hotspots?threshold=${threshold}`)
...
export function useRepositoryAnalysis(repositoryId: string) {
  ...
  const isLoading = analysisResults.isLoading || hotspots.isLoading || fileMetrics.isLoading
  const isError = analysisResults.isError || hotspots.isError || fileMetrics.isError
...
export function useAnalysisWithPolling(repositoryId: string, jobId?: string) {
  ...
  const shouldPoll = status.data?.status === 'running' || status.data?.status === 'pending'
